...
// style
import style from "../../styles/secondContent/componentStyle/LastUpdateContainer.module.scss";
...
export default function LastUpdateContainer({ idStory, image, title, vote }: Props) {
    const { push } = useRouter();

    function handleToDetail() {
        push(`/${slugify(title)}/${idStory}`);
    }

    return (
        <div className={style.containerStory} onClick={handleToDetail}>
            <div className={style.containerStory__imageContainer}>
                <Image 
                    className={style.containerStory__imageContainer__image}
                    src={image} 
                    alt={title} 
                    fill
                />
            </div>
            <div className={style.containerStory__detailContainer}>
                <span className={style.containerStory__detailContainer__title}>{title}</span>
                <div className={style.containerStory__detailContainer__voteContainer}>
                    ...
                    <span className={style.containerStory__detailContainer__voteContainer__vote}>{formatNumber(vote)}</span>
                </div>
            </div>
        </div>
    )
