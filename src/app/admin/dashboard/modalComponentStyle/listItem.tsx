// technical
import React, { ChangeEvent } from "react";
import Image from "next/image";

// style
import style from "./styles/componentStyle/ListItem.module.scss";

type Props = {
    name: string;
    profile: string;
    inputRef: (el: HTMLInputElement | null) => void;
    handleOnChangeProfile: (e: ChangeEvent<HTMLInputElement>) => void;
    handleOnClickProfile: () => void;
    handleOnChangeName: (e: React.ChangeEvent<HTMLInputElement>) => void;
    removeProfile: () => void;
}

export default function ListItem(
    {
        name, 
        profile, 
        inputRef,
        handleOnChangeProfile, 
        handleOnClickProfile, 
        handleOnChangeName,
        removeProfile
    }: Props
) {
    return (
        <div className={style.listItem}>
            <div className={style.listItem__profileContainer}>
                {/* <div className={style.listItem__profileContainer__profile} onClick={handleOnClickProfile}>{name !== "" ? name.charAt(0).toUpperCase() : "?"}</div> */}

                <Image 
                    src={profile}
                    alt="add image profile"
                    width={40}
                    height={40}
                    className={style.listItem__profileContainer__profile}
                    onClick={handleOnClickProfile}
                />
                <input 
                    ref={inputRef}
                    type="file"
                    onChange={handleOnChangeProfile}
                    hidden
                />
            </div>
            <input className={style.listItem__username} type="text" value={name} onChange={handleOnChangeName} />
            <svg onClick={removeProfile} className={style.listItem__remove} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="Bin-1--Streamline-Ultimate" height="24" width="24">
                <desc>
                    Bin 1 Streamline Icon: https://streamlinehq.com
                </desc>
                <g id="Bin-1--Streamline-Ultimate.svg">
                    <path d="M19.45 7.5H4.55a0.5 0.5 0 0 0 -0.5 0.54l1.28 14.14a2 2 0 0 0 2 1.82h9.34a2 2 0 0 0 2 -1.82L20 8a0.5 0.5 0 0 0 -0.5 -0.54Zm-9.2 13a0.75 0.75 0 0 1 -1.5 0v-9a0.75 0.75 0 0 1 1.5 0Zm5 0a0.75 0.75 0 0 1 -1.5 0v-9a0.75 0.75 0 0 1 1.5 0Z" fill="#FFFFFF" strokeWidth="1"></path>
                    <path d="M22 4h-4.75a0.25 0.25 0 0 1 -0.25 -0.25V2.5A2.5 2.5 0 0 0 14.5 0h-5A2.5 2.5 0 0 0 7 2.5v1.25a0.25 0.25 0 0 1 -0.25 0.25H2a1 1 0 0 0 0 2h20a1 1 0 0 0 0 -2ZM9 3.75V2.5a0.5 0.5 0 0 1 0.5 -0.5h5a0.5 0.5 0 0 1 0.5 0.5v1.25a0.25 0.25 0 0 1 -0.25 0.25h-5.5A0.25 0.25 0 0 1 9 3.75Z" fill="#FFFFFF" strokeWidth="1"></path>
                </g>
            </svg>
        </div>
    )
}