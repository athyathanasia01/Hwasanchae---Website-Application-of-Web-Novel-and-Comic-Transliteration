...
export default function DonatesContent() {
    ...
    async function uploadImageLogo(file: File) {
        ...
        const urlLink = process.env.CLOUDINARY_UPLOAD_IMAGE_LINK!;
    ...
    function handleOnChangeName(e: ChangeEvent<HTMLInputElement>, index: number) {
        setDonates(prev => prev.map((donate, i) => (
            i === index ? { ...donate, name: e.target.value } : donate
        )));
    }

    function handleOnChangeLink(e: ChangeEvent<HTMLInputElement>, index: number) {
        setDonates(prev => prev.map((donate, i) => (
                ...
                :
                    <ul className={style.contentPage__unorderList}>
                        {donates.map((donate, index) => (
                            <li className={`${style.contentPage__unorderList__list} ${style[`contentPage__unorderList__list--donate`]}`}>
